...
month <- as.character(asthma_sample$month)                    # 추출된 정보가 date 포맷이기 때문에 charatcter로 변경하기

asthma1 <- cbind(asthma1, month, stringsAsFactors = FALSE)      # 따로 만들어진 변수 month를 원본 cold1에 결합하기.

str(asthma1)      # 처음에 date format인 month가 chr로 변경된 것을 확인/                                              
